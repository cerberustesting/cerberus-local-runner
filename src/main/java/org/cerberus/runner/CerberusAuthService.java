...
final class CerberusAuthService {
    ...
    AuthHeader authHeader() {
        ...
        if ("oauth".equals(mode)) return new AuthHeader("Authorization", "Bearer " + config.get("cerberus.auth.oauth.accessToken"));
        return null;
    }
    ...
    synchronized TestResult saveApiKey(String cerberusUrl, String apiKey, String login) throws IOException {
        requireNonBlank(cerberusUrl, "Cerberus URL");
        requireNonBlank(apiKey, "API key");
        requireNonBlank(login, "Cerberus login");
        ...
        // Cerberus doesn't expose a "whoami" for API keys - the key is 1:1 tied to a user server-side,
        // but resolving that login back to us would need a dedicated endpoint that doesn't exist yet,
        // so the user types it once here instead (needed to name this runner's own "local-runner-{login}" robot).
        config.set("cerberus.auth.login", login.trim());
        config.save();
    ...
    synchronized String startOAuth(String cerberusUrl, String keycloakUrl, String realm, String clientId) throws IOException {
        requireNonBlank(cerberusUrl, "Cerberus URL");
        requireNonBlank(keycloakUrl, "Keycloak URL");
        requireNonBlank(realm, "Realm");
        requireNonBlank(clientId, "Client ID");

        String normalizedKeycloak = normalizeUrl(keycloakUrl);
    ...
    /** Extracts one top-level string/number field from a small, flat JSON object (no JSON library on the classpath). */
